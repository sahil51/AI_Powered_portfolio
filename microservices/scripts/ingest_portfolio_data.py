...
from infrastructure.database.session import Base, DatabaseEngine
from monitoring.logger import logger
from rag.embeddings.service import EmbeddingService


async def _fetch_portfolio_data(session: AsyncSession) -> list[dict]:
    items = []
...
async def main():
    logger.info("Starting portfolio data ingestion...")

    engine = DatabaseEngine()
    await engine.initialize()
    session_factory: async_sessionmaker[AsyncSession] = engine.session_factory
    ...
    async with session_factory() as session:
        items = await _fetch_portfolio_data(session)
        logger.info(f"Found {len(items)} items to ingest")

        existing = await session.execute(
            sa_text("SELECT id FROM knowledge_documents WHERE title = 'portfolio_data' AND is_deleted = FALSE")
        )
        ...
        if existing_doc:
            ...
            await session.execute(
                sa_text("""
                    ...
                    VALUES (:id, FALSE, 1, 'portfolio_data', 'db_ingest', 'active', 'en', :now, :now)
                """),
                {"id": str(doc_id), "now": now},
            )

        chunk_index = 0
        for item in items:
            chunks = _chunk_text(item["text"], 2000)
            for chunk_text in chunks:
                try:
                    embedding = await embedder.embed_text(chunk_text)
                    chunk_meta = json.dumps(item["tags"])
                    await session.execute(
                        sa_text("""
    ...
    logger.info(f"Ingestion complete. {chunk_index} chunks created/updated.")
