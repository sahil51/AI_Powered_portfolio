...
from application.retrieval.metrics import RetrievalMetrics


class RetrievalHealthStatus(str, Enum):
    HEALTHY = "healthy"
    DEGRADED = "degraded"
    UNHEALTHY = "unhealthy"
...
class RetrievalHealth:
    status: RetrievalHealthStatus = RetrievalHealthStatus.HEALTHY
...
class RetrievalHealthChecker:
    ...
    def check(self, metrics: RetrievalMetrics | None = None) -> RetrievalHealth:
        errors: list[str] = []
        status = RetrievalHealthStatus.HEALTHY
        ...
        if len(self._consecutive_failures) >= self._max_consecutive_failures:
            ...
            status = RetrievalHealthStatus.UNHEALTHY
    ...
    def record_failure(self, error: str) -> None:
        ...
        if len(self._consecutive_failures) > self._max_consecutive_failures * 2:
            self._consecutive_failures = self._consecutive_failures[-self._max_consecutive_failures:]
